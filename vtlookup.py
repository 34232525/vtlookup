...
def fileexists(filepath):
	try:
		if os.path.isfile(filepath):
			...
			print ("There is no file at:" + str(filepath))
			exit()
	except Exception as e:
			print (e)
...
def VT_Request(key, hash, output):
	...
	if response == 0:
		print (str(hash) + " is not in Virus Total")
		file = open(output,'a')
		...
		if positives == 0:
			print (str(hash) + ' is not malicious')
			file = open(output,'a')
			file.write(hash + ' is not malicious')
			file.write('\n')
			file.close()
		else:
			print (str(hash) + ' is malicious')
			file = open(output,'a')
			file.write(hash + ' is malicious. Hit Count:' + str(positives))
			file.write('\n')
			file.close()
	else:
		print (srt(hash) + ' could not be searched. Please try again later.')
# execute the program
